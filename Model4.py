...
VRP = np.array(VRP, dtype="object")

# Parsing the data structure
N = np.array([row[0] for row in VRP])  # Nodes (including depot)
C = np.array(N[1:-1])  # Customers/Locations
V = [0, 1, 2]  # Vehicles
Q = 200  # Vehicle capacity
d = np.array([row[3] for row in VRP])  # Demand
s = np.array([row[4] for row in VRP])  # Service time
r = []
e = []
NUM_TW = np.array([row[5] for row in VRP])  # Number of time windows per location
for i in N:
    ri = []
    for k in range(0, NUM_TW[i] * 2, 2):
        ri.append(VRP[i][6 + k])
    r.append(ri)
for i in N:
    ei = []
    for k in range(0, NUM_TW[i] * 2, 2):
        ei.append(VRP[i][7 + k])
    e.append(ei)
mx = np.array([row[1] for row in VRP])  # X-position of nodes
my = np.array([row[2] for row in VRP])  # Y-position of nodes
...
for j in C:
    ...
    # m.addConstr(quicksum(y[j, t, v] for t in range(NUM_TW[j]) for v in V) <= len(V))

# Link z and y variables, vehicle v visits j only once (one TW)
for j in N:
    for v in V:
...
# for j in C:
#     m.addConstr(quicksum(d[j] * z[j, v] for v in V) <= Q)
...
if m.status == GRB.Status.OPTIMAL:
    ...
    for v in V:
        ...
        for i in route_sorted:
            loc = i[0]
            i.append(load)
            load += d[loc] * f[loc, v].X
            if loc == len(C) + 1:
                i[0] = 'Depot'
        conc_str = 'Loc-Depot>' + '>'.join(
            'Loc' + str(a) + ' visited at T' + str(b) + ' with load Q' + str(c) for a, b, c in route_sorted)
